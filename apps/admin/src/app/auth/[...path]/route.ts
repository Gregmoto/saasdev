/**
 * Runtime proxy for /auth/* → backend API.
 *
...
async function proxy(
  ...
  const upstream = `${apiBase}/auth/${path.join("/")}${req.nextUrl.search}`;

  // Forward only safe headers to the upstream.
  ...
  // Build the response, forwarding all headers (especially Set-Cookie).
