...
export function LegalVersionSwitcher({
  ...
  async function handleVersionChange(newId: string) {
    ...
    setFetchError(false);
    try {
      const res = await fetch(`${API}/api/cms/legal/terms/${newId}`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-8 pb-6 border-b border-stone-100">
        ...
        {versions.length > 1 && (
          <div className="flex items-center gap-2">
            <label htmlFor="terms-version-select" className="text-xs text-stone-500 whitespace-nowrap">
              Visa version:
            </label>
            <select
              id="terms-version-select"
              value={selectedId}
